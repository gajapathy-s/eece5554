...
bag_filename="walkingRTK"


def GNGGA(gpgga_string):
    ...
    if title == "$GNGGA":
        ...
        cmsg.latitude = float(fields[2])
        cmsg.gngga_read=gpgga_string
        cmsg.hdop = float(fields[8])
        cmsg.fix_quality= int(fields[6])
...
if __name__ == '__main__':
    ...
    GPS_pub = rospy.Publisher("rtk_gnss", Customrtk, queue_size=10)
    rate = rospy.Rate(10)
...
try:
    try:
        while not rospy.is_shutdown():
            cmsg = Customrtk()
            gngga_read =(port.readline().decode('utf-8').strip())
            cmsg.gngga_read= str(gngga_read)
           # print((cmsg.gngga_read))
            ...
            if cmsg.gngga_read == ' ':
                rospy.logwarn("No data")
                    ...
                    try: 
                     [cmsg, title] = GNGGA(cmsg.gngga_read)
                     ...
                     if title == "$GNGGA":
                            ...
                            bag.write("rtk_gnss", cmsg)
                           
                            rospy.sleep(sleep_time)
                
                    except Exception as e :
                        rospy.logwarn("Data exception: " + gngga_read + "\nException: " + str(e))
                        continue        
